...
const setImages = () => {
    ...
    for (let i = 0; i < sceneInfo.videoImageCount; i++) {
        ...
        imgElem.src = `${require(`img/video/IMG_${1000 + i}.webp`).default}`;
...
const Home2 = () => {
    ...
    function loop () {
        delay = delay + ((currentRatioY-delay) * 0.1)
        rafId = requestAnimationFrame(loop);
        value = playAnimation(currentRatioY, currentScene);
        console.log(value.sequence)

        if( Math.abs(currentRatioY - delay) < 0.01 ) {
